...
func Test_DecodeNameAndTags(t *testing.T) {
	var tests = []struct {
		...
		{test: "metric only", str: "cpu", name: "cpu", schema:"measurement"},
		{test: "metric with single series", str: "cpu.server01", name: "cpu",
			schema:"measurement.hostname", tags: map[string]string{"hostname": "server01"}},
		{test: "metric with multiple series", str: "cpu.us-west.server01", name: "cpu",
		...
		{test: "name shorter than schema", str: "foo", schema: "measurement.A.B.C",
			tags: make(map[string]string), name: "foo"},
	}
	...
	for _, test := range tests {
		...
		p.FieldNames = strings.Split(test.schema, p.Separator)
		p.IgnoreUnnamed = test.ignore
